<!--
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
    <script src="https://static.codepen.io/assets/common/stopExecutionOnTimeout-de7e2ef6bfefd24b79a3f68b414b87b8db5b08439cac3f1012092b2290c719cd.js"></script>

    <script src=" https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js"> </script> -->
<!-- JS here -->
<script src="<?php echo e(url('js/vendor/modernizr-3.5.0.min.js')); ?>"></script>
<script src="<?php echo e(url('js/vendor/jquery-1.12.4.min.js')); ?>"></script>
<script src="<?php echo e(url('js/popper.min.js')); ?>"></script>
<script src="<?php echo e(url('js/bootstrap.min.js')); ?>"></script>
<script src="<?php echo e(url('js/owl.carousel.min.js')); ?>"></script>
<script src="<?php echo e(url('js/isotope.pkgd.min.js')); ?>"></script>
<script src="<?php echo e(url('js/ajax-form.js')); ?>"></script>
<script src="<?php echo e(url('js/waypoints.min.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.counterup.min.js')); ?>"></script>
<script src="<?php echo e(url('js/imagesloaded.pkgd.min.js')); ?>"></script>
<script src="<?php echo e(url('js/scrollIt.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.scrollUp.min.js')); ?>"></script>
<script src="<?php echo e(url('js/wow.min.js')); ?>"></script>
<script src="<?php echo e(url('js/nice-select.min.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.slicknav.min.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.magnific-popup.min.js')); ?>"></script>
<script src="<?php echo e(url('js/plugins.js')); ?>"></script>
<script src="<?php echo e(url('js/gijgo.min.js')); ?>"></script>
<script src="<?php echo e(url('js/slick.min.js')); ?>"></script>



<!--contact js-->
<script src="<?php echo e(url('js/contact.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.ajaxchimp.min.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.form.js')); ?>"></script>
<script src="<?php echo e(url('js/jquery.validate.min.js')); ?>"></script>
<script src="<?php echo e(url('js/mail-script.js')); ?>"></script>


<script src="<?php echo e(url('js/main.js')); ?>"></script>
<script>
    $('#datepicker').datepicker({
        iconsLibrary: 'fontawesome',
        icons: {
            rightIcon: '<span class="fa fa-caret-down"></span>'
        }
    });
</script>
<?php /**PATH C:\xampp\htdocs\New folder\PA-III-02-2020\resources\views/layout/front/includes/script.blade.php ENDPATH**/ ?>